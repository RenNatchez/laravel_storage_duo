...
@section('content')
    ...
    <form action="{{route('membre.store')}}" method="POST" enctype="multipart/form-data" class="container">
        ...
        <div class="form-group">
            ...
            <input type="number" id="age" name="age" class="form-control @error('age') is-invalid @enderror" value="{{ old('age') }}">
            @error('age')
                <span class="invalid-feedback"> <strong>{{ $message }}</strong></span>
            @enderror
        ...
        <select class="form-control @error('genre') is-invalid @enderror" name="genre_id" value="{{ old('genre_id') }}">
